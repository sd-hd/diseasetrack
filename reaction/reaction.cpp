...
void Reaction::AcceptClusters(
  ...
  for (unsigned int i = 0; i < std::get<1>(_clusters).size(); i++) {
    ...
    try {
      ...
      // boost::property_tree::ptree &BLEAttenuation_tree =
      // pt.get_child("BLEAttenuation"); float BLEAttenuation =
      // BLEAttenuation_tree.get_value<float>();
      boost::property_tree::ptree &Timestamp_tree = pt.get_child("Timestamp");
      int Timestamp = Timestamp_tree.get_value<int>();
      // boost::property_tree::ptree &GPSLatitude_tree =
      // pt.get_child("GPSLatitude"); float GPSLatitude =
      // GPSLatitude_tree.get_value<float>(); boost::property_tree::ptree
      // &GPSLongitude_tree = pt.get_child("GPSLongitude"); float GPSLongitude =
      // GPSLongitude_tree.get_value<float>();
      std::tuple<std::string, std::string, UINT64> temp = std::make_tuple(
          UUID, crypto->trunchashbase64(base64_decode(std::get<0>(_clusters))),
          Timestamp);
      toDB.push_back(temp);
